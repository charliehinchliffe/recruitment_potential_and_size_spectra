...
Plot
```{r}
output_data <- readRDS("output/simulations_biomass_ratio_age15toage5.rds")
...
Fig_1 <- ggplot(output_data, aes(RP, ratio)) +
  ...
  geom_vline(xintercept = 1, color = "red", size=0.5, alpha = 0.5) +
  theme_bw()+
...
v <- 0.15*sin(t) + 0.25
offv <- 0.15*sin(t+2.5) + 0.25

y <- (v)#M
y2 <- (offv) #G
y3 <- log(y/y2) #M/G

plot(y~t, ylim=c(-1.5, 1.5))
lines(y~t, col = "green")
lines(y2~t, col = "red")
lines(y3~t, col = "blue")
```
...
output_data2 <- cbind(t,z,g) %>%  as.data.frame()

output_data3 <- cbind(t,z,g) %>%  as.data.frame()


output_data <- rbind.data.frame(output_data, output_data2, output_data3)
...
```{r}
output_data <- readRDS("output/simulations_sine_function.rds")
...
hatches_per_day <- c(rep(10, times = 500),
                     rep(20, times = 500),
                     rep(50, times = 500),
                     rep(100, times = 500),
                     rep(200, times = 500),
...
Read output 
```{r}
output_data <- readRDS("output/simulations_R_variation2.rds")
```
...
```{r}
output_data <- readRDS("output/simulations_Z_variation2.rds")
```
...
```{r}
output_data <- readRDS("output/simulations_g_variation2.rds")
```
...
```{r}
output_data <- readRDS("output/simulations_Age_variation2.rds")
...
Make long format data
```{r}
output_data <- readRDS("output/simulations_size_measurement_error2.rds")
