...
z <- runif(N_iter, min = 0.1, max = 0.4) 
g <- runif(N_iter, min = 0.1, max = 0.4) 
...
#saveRDS(output_data, "outputs/simulations_biomass_ratio_age15toage5_final.rds")
```
...
```{r}
output_data <- readRDS("outputs/simulations_biomass_ratio_age15toage5_final.rds")
...
z <- runif(N_iter, min = 0.1, max = 0.4) #1000
g <- runif(N_iter, min = 0.1, max = 0.4) #1000
t <- seq(1, N_iter, by = 1)
...
# Now fit stan model
fit_size <- sampling(ret_size, data = data_for_stan_size_growth, iter = 999, chains = 3, cores = 3) #chains = 3
...
# fit abund model
fit_abund <- sampling(ret_abund, data = data_for_NASS, iter = 999, chains = 3, cores = 3)
...
start <- list(W0 = 0.01 , g = 0.4)  ### setting start parameters from above
...
#saveRDS(output_data, "outputs/simulations_runif_z_g_final.rds")
```

Plot output

```{r}
output_data <- readRDS("outputs/simulations_runif_z_g_final.rds")

output_data2 <- output_data #%>% subset(t < 107)



output_data2$RP <- output_data2$z/output_data2$g
output_data2$logRP <- log(output_data2$RP)
output_data2$abund_RP <- (output_data2$median_b)-1

output_data2 <- output_data2 %>% 
  select("t", "z", "g", "RP", "logRP", "size_Z_median", "size_g_median",  "size_RP_median",  "abund_RP", "LM_MG", "LM_M", "LM_G", "wlr_M", "wlr_MG", "crcb_M", "crcb_MG") %>% 
  rename(size_Z = size_Z_median,
         size_g = size_g_median,
         size_RP = size_RP_median,
         ) #taking median sizedist estimate instead on mean
...
```{r}
rel_growth_plot <- growth_data2 %>% ggplot(aes(set_g, (estimate-set_g)/set_g)) + 
  geom_abline(intercept = 0, slope = 0, linetype="dashed")+
  geom_point2(alpha = 0.5, size = 1) +
  geom_smooth(method = "lm", se = T, col = "black") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") +
  ylab("Relative Error") +
  xlab(bquote("True "~bar(g)~"")) +
  facet_wrap(~method, nrow = 1) +
  ggtitle("A") #+
  #scale_color_manual(values =  pal_1_1)

rel_mort_plot <- mort_data2 %>% ggplot(aes(set_z, (estimate-set_z)/set_z)) + 
  geom_abline(intercept = 0, slope = 0, linetype="dashed") +
  geom_point2(alpha = 0.5, size = 1) +
  geom_smooth(method = "lm", se = T, col = "black") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") +
  ylab("Relative Error") +
  xlab(expression("True "~bar(z)~"")) +
  facet_wrap(~method, nrow = 1) +
  ggtitle("B") #+
  #scale_color_manual(values =  pal_2_1)

rel_RP_plot <- RP_data2 %>% ggplot(aes(set_RP, (estimate-set_RP)/set_RP)) + 
  geom_abline(intercept = 0, slope = 0, linetype="dashed") +
  geom_point2(alpha = 0.5, size = 1) +
  geom_smooth(method = "lm", se = T, col = "black") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom") +
  xlab(expression(~"True "~bar(z)/bar(g))) +
  ylab("Relative Error") +
  facet_wrap(~method, nrow = 1) +
  ggtitle("C") +
...
Fig_2 <- rel_growth_plot / rel_mort_plot / rel_RP_plot
Fig_2
```

Tables
```{r}
mort_rmse_table <- mort_data %>% 
  group_by(model) %>%  
  summarise(Model = method,
            n = n(),
         rm2e = RMSE(estimate, set_z)
         ) %>% 
  distinct() %>% 
  as.data.frame() %>% 
  arrange(Model) %>% select(Model, rm2e)

mort_rmse_table 

growth_rmse_table <- growth_data %>% 
  group_by(model) %>%  
  summarise(Model = method,
            n = n(),
         rm2e = RMSE(estimate, set_g)
         ) %>% 
  distinct() %>% 
  as.data.frame() %>% 
  arrange(Model) %>% select(Model, rm2e)
...
RP_rmse_table <- RP_data %>% 
  group_by(model) %>%  
  summarise(Model = method,
            n = n(),
         rm2e = RMSE(estimate, set_z/set_g)
         ) %>% 
  distinct() %>% 
  as.data.frame() %>% 
  arrange(Model) %>% select(Model, rm2e)
...
# Now fit stan model
fit_size <- sampling(ret_size, data= data_for_stan_size_growth, iter = 999, chains = 3, cores = 3)
...
#saveRDS(output_data, "outputs/simulations_R_variation_final.rds")
```

Read output 
```{r}
output_data <- readRDS("outputs/simulations_R_variation_final.rds")
```
...
data_plotting <- output_data2 %>% select(hatches_per_day, size_RP_median, abund_RP, LM_MG, wlr_MG, crcb_MG) %>% 
  rename(
         size_RP = size_RP_median
         ) #taking median sizedist estimate instead on mean
...
summary_table %>% subset(R <= 50)

R_whole <- summary_table %>%
  #subset(Variance < 0.1) %>% 
  mutate(mean = round(mean,3),
         sd = round(sd,3),
         rm2e = round(rm2e,3)) %>% 
  select(Model, R, mean, sd, rm2e)

write.csv(R_whole, "outputs/R_sensitivity.csv")
...
#saveRDS(output_data, "outputs/simulations_Z_variation_final.rds")
```

Read output

```{r}
output_data <- readRDS("outputs/simulations_Z_variation_final.rds")
```
...
data_plotting <- output_data2 %>% select(variance, size_RP_median, abund_RP, LM_MG, wlr_MG, crcb_MG) %>% rename(z_var = variance,
        size_RP = size_RP_median
         ) #taking median sizedist estimate instead on mean
...
summary_table %>% subset(Variance < 0.1)

Z_whole <- summary_table %>%
  #subset(Variance < 0.1) %>% 
  mutate(mean = round(mean,3),
         sd = round(sd,3),
         rm2e = round(rm2e,3)) %>% 
  select(Model, Variance, mean, sd, rm2e)

write.csv(Z_whole, "outputs/Z_var_sensitivity.csv")
...
#saveRDS(output_data, "outputs/simulations_g_variation_final.rds")
```

### Read output

```{r}
output_data <- readRDS("outputs/simulations_g_variation_final.rds")
```

Make long format data
```{r}

output_data2 <- readRDS("outputs/simulations_g_variation_final.rds")

output_data2$abund_RP <- (output_data$median_b)-1

data_plotting <- output_data2 %>% select(variance, size_RP_median, abund_RP, LM_MG, wlr_MG, crcb_MG) %>%
  rename(g_var = variance,
        size_RP = size_RP_median)
...
#saveRDS(output_data, "outputs/simulations_Age_variation_final.rds")
```
...
```{r}
output_data <- readRDS("outputs/simulations_Age_variation_final.rds")

output_data2 <- output_data

output_data2$abund_RP <- (output_data2$median_b)-1

data_plotting <- output_data2 %>% select(variance, size_RP_median, abund_RP, LM_MG, wlr_MG, crcb_MG) %>% rename(age_var = variance,                                                                                            size_RP = size_RP_median)
...
write.csv(age_whole, "outputs/age_var_sensitivity.csv")
...
#saveRDS(output_data, "outputs/simulations_size_measurement_error_final.rds")
```

Make long format data
```{r}
output_data <- readRDS("outputs/simulations_size_measurement_error_final.rds")

output_data2 <- output_data

output_data2$abund_RP <- (output_data2$median_b)-1

data_plotting <- output_data2 %>% select(variance, size_RP_median, abund_RP, LM_MG, wlr_MG, crcb_MG) %>% rename(size_var = variance,
        size_RP = size_RP_median)
...
write.csv(size_whole, "outputs/size_var_sensitivity.csv")
...
## Fig. 4: measurement error

```{r}
Fig_4 <- age_var_plot / size_var_plot
Fig_4
```
