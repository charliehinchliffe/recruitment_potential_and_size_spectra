...
### Fig. X: varying growth and mortality
```{r}
...
Fig_X <- (growth_plot | mort_plot) / RP_plot
...
rel_growth_plot / rel_mort_plot / rel_RP_plot
```
...
```{r}
Fig_2 <-(cor_growth_plot | res_growth_plot) / (cor_mort_plot | res_mort_plot) / (cor_RP_plot | res_RP_plot)
...
#saveRDS(output_data, "outputs/simulations_g_variation_ReRun.rds")
...
output_data <- output_data %>% 
  subset(variance != 0.1) %>% 
  subset(variance != 0.05)
tail(output_data)

output_data2 <- readRDS("outputs/simulations_g_variation_ReRun.rds")

output_data2 <- output_data2 %>% 
  subset(variance != 0) %>% 
  subset(variance != 0.005) %>% 
  subset(variance != 0.01) %>% 
  subset(variance != 0.02) %>% 
...
Make long format data
```{r}
output_data2 <- rbind(output_data, output_data2)
...
### Size error with expanded binwidth.
...
saveRDS(output_data, "outputs/simulations_size_measurement_error_ExpandedBinwidth.rds")
```
